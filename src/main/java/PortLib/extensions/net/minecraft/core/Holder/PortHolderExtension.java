...
public class PortHolderExtension {
    public static <T> boolean is(@This Holder<T> thiz, Holder<T> holder) {
    ...
    public static <T> String getRegisteredName(@This Holder<T> thiz) {
        return thiz.unwrapKey().map(key -> key.location().toString()).orElse("[unregistered]");
    }

    public static <T> @Nullable ResourceKey<T> getKey(@This Holder<T> thiz) {
        return thiz.unwrapKey().orElse(null);
    }

    public static <T> HolderLookup.@Nullable RegistryLookup<T> unwrapLookup(@This Holder<T> thiz) {
