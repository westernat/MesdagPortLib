...
public interface PortStreamCodec<B, V> {
    ...
    @Diff
    default StreamCodec<B, V> unwrap() {
        return new StreamCodec<>() {
            @Override
            public V decode(B buffer) {
                return PortStreamCodec.this.decode(buffer);
            }

            @Override
            public void encode(B buffer, V value) {
                PortStreamCodec.this.encode(buffer, value);
            }
        };
    }

    @Diff
    static <B, V> PortStreamCodec<B, V> wrap(StreamCodec<B, V> codec) {
        return new Delegate<>(codec);
    }
    ...
    interface PortCodecOperation<B, S, T> {
        ...
        @Diff
        static <B, S, T> PortCodecOperation<B, S, T> wrap(StreamCodec.CodecOperation<B, S, T> operation) {
            return codec -> PortStreamCodec.wrap(operation.apply(codec.unwrap()));
        }
    }

    @Diff
    record Delegate<B, V>(StreamCodec<B, V> codec) implements PortStreamCodec<B, V> {
        @Override
        public void encode(B buffer, V packet) {
