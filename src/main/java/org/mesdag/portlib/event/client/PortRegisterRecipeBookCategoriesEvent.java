...
public class PortRegisterRecipeBookCategoriesEvent extends PortEvent<RegisterRecipeBookCategoriesEvent> {
    ...
    public void registerRecipeCategoryFinder(RecipeType<?> type, BiFunction<PortIdentifier, Recipe<?>, RecipeBookCategories> lookup) {
        e.registerRecipeCategoryFinder(type, holder -> lookup.apply(holder.id().wrap(), holder.value()));
    }
