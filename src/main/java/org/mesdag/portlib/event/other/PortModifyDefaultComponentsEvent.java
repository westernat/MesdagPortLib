...
public class PortModifyDefaultComponentsEvent extends Event implements IModBusEvent {
    ...
    public void modify(ItemLike item, Consumer<PortDataComponentPatch.PortBuilder> patch) {
        ...
        if (!compPatch.isEmpty()) {
            ...
            for (Map.Entry<PortDataComponentType<?>, Optional<?>> entry : portItem.portlib$getComponents().entrySet()) {
                builder2.set((PortDataComponentType) entry.getKey(), entry.getValue().orElse(null));
            }
