package org.mesdag.portlib.event.entity.client.event;


import net.minecraft.resources.ResourceLocation;
import net.minecraft.world.entity.EntityType;
import net.neoforged.neoforge.client.event.RegisterEntitySpectatorShadersEvent;
import org.mesdag.portlib.diff.Diff;
import org.mesdag.portlib.event.PortEvent;
import org.mesdag.portlib.event.PortEventHooks;

public class PortRegisterEntitySpectatorShadersEvent extends PortEvent {
    private final RegisterEntitySpectatorShadersEvent e;

    @Diff
    public PortRegisterEntitySpectatorShadersEvent(RegisterEntitySpectatorShadersEvent e) {
        super(e);
        this.e = e;
    }

    public void register(EntityType<?> entityType, ResourceLocation shader) {
        e.register(entityType, shader);
    }

    static {
        PortEventHooks.register(RegisterEntitySpectatorShadersEvent.class, PortRegisterEntitySpectatorShadersEvent.class, PortRegisterEntitySpectatorShadersEvent::new);
    }
}